...
<%
    ...
    String adminName = sess.getAttribute("userName") != null ?
        (String) sess.getAttribute("userName") : "Platform Admin";
...
%></h3> 
        ...
        <div class="admin-card mb-4" style="border: 1px solid #10B981;">
            <div class="admin-card-header" style="background: rgba(16, 185, 129, 0.05);">
                <h5 class="text-dark fw-bold m-0 d-flex align-items-center gap-2">
                    <i class="bi bi-cpu-fill text-success"></i> Intelligent Emergency Recommendation Radar
                </h5>
                <span class="badge bg-success">Top 5 Ranked Matches</span>
            </div>
                ...
                <table class="admin-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th>Entity Name</th>
                            <th>Type Role</th>
                            <th>Jurisdiction Proximity</th>
                            <th style="width: 200px;">AI Match Score Matrix</th>
                        </tr>
                    </thead>
                    <tbody>
                        <%
                        try {
                            // Instant execution call from our newly developed USERDao
                            com.carelink.dao.USERDao recommendationEngine = new com.carelink.dao.USERDao();
                            
                            // Testing parameters: Querying for an emergency constraint inside Indore location
                            int mockEmergencyId = 102; 
                            String requiredCity = "Indore";
                            String requiredSkills = "volunteer"; 
                            String requiredResources = "ngo";
                            
                            java.util.List<com.carelink.dao.USERDao.RecommendationMatch> topMatches = 
                                recommendationEngine.getIntelligentRecommendations(mockEmergencyId, requiredCity, requiredSkills, requiredResources);
                            
                            if (topMatches == null || topMatches.isEmpty()) {
                        %>
                            <tr>
                                <td colspan="5" class="text-center text-muted py-4">No active candidates match proximity or capacity bounds.</td>
                            </tr>
                        <%
                            } else {
                                int matchRank = 1;
                                for (com.carelink.dao.USERDao.RecommendationMatch match : topMatches) {
                        %>
                            <tr>
                                <td><span class="badge bg-dark rounded-pill px-2.5 py-1">Rank #<%= matchRank++ %></span></td>
                                <td><strong><%= match.name %></strong></td>
                                <td>
                                    <span class="status-badge <%= "ngo".equalsIgnoreCase(match.role) ? "pending" : "approved" %>">
                                        <%= match.role.toUpperCase() %>
                                    </span>
                                </td>
                                <td><i class="bi bi-geo-alt-fill text-secondary me-1"></i><%= match.city %></td>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="progress w-100" style="height: 6px; background-color: #f1f5f9; border-radius:3px;">
                                            <div class="progress-bar bg-success" role="progressbar" style="width: <%= match.matchScore %>%"></div>
                                        </div>
                                        <strong class="text-dark small"><%= (int)match.matchScore %>%</strong>
                                    </div>
                                </td>
                            </tr>
                        <%
                        ...
                        %>
                            <tr>
                                <td colspan="5" class="text-danger">Recommendation Pipeline Exception: <%= ex.getMessage() %></td>
                            </tr>
                        <%
                        }
                        %>
                    ...
                    <thead> 
                     ...
                     <tbody>
						...
						try {
                            ...
                            // ✅ SAFE REUSE: Reusing variables without duplicate local declaration errors
                            PreparedStatement psShared = con.prepareStatement(sql); 
						    ResultSet rsShared = psShared.executeQuery(); 
						    while(rsShared.next()) {
						%>
						<tr>
							<td><strong><%= rsShared.getString("org_name") %></strong></td> 
							<td><%= rsShared.getString("email") %></td> 
							<td><%= rsShared.getString("city") %></td> 
							<td><%= rsShared.getTimestamp("created_at") %></td> 
							<td><span class="status-badge <%= rsShared.getString("status").toLowerCase() %>"><%= rsShared.getString("status") %></span></td> 
            ...
            <div class="table-responsive">
                <table class="admin-table">
                    ...
                    <tbody>
						...
						%>
						<tr>
							<td><strong><%= rsShared.getString("full_name") %></strong></td> 
							<td><%= rsShared.getString("email") %></td> 
							<td><%= rsShared.getString("role") %></td> 
							<td><%= rsShared.getString("city") %></td> 
                    ...
                    <tbody>
						...
						try {
							...
							while(rsShared.next()) { 
                                ...
                                try {
                                    aiStatus = rsShared.getString("ai_status");
                                    if (aiStatus == null) aiStatus = "NOT_ANALYZED";
                                } catch(SQLException ex) {}

                                try {
                                    aiScore = rsShared.getInt("ai_confidence_score");
                                } catch(SQLException ex) {}

                                try {
                                    extRegNo = rsShared.getString("extracted_reg_no");
                                    if (extRegNo == null) extRegNo = "N/A";
                                } catch(SQLException ex) {}

                                try {
                                    aiSummary = rsShared.getString("ai_analysis_summary");
                                    if (aiSummary == null) aiSummary = "";
                                } catch(SQLException ex) {}

                                try {
                                    passedChecks = rsShared.getString("ai_passed_checks");
                                    if (passedChecks == null) passedChecks = "";
                                } catch(SQLException ex) {}

                                try {
                                    criticalIssues = rsShared.getString("ai_critical_issues");
                                    if (criticalIssues == null) criticalIssues = "";
                                } catch(SQLException ex) {}
						%>
						<tr data-status="<%= rsShared.getString("status").toLowerCase() %>"> 
							<td><strong><%= rsShared.getString("org_name") %></strong></td> 
							<td><%= rsShared.getString("registration_number") %></td> 
							<td><%= rsShared.getString("email") %></td> 
							<td><%= rsShared.getString("city") %></td> 
							<td>
                                <div class="d-flex flex-column gap-1">
                                    <a href="<%= rsShared.getString("document_path") 
                                    %>" target="_blank" class="btn-cert text-center py-1.5"><i class="bi bi-file-earmark-pdf-fill"></i> View PDF</a>
                                    
                                    <form action="verifyNGO" method="POST" class="m-0 mt-1">
                                        <input type="hidden" name="ngo_id" value="<%= rsShared.getInt("id") %>">
                        
                                        <input type="hidden" name="document_path" value="<%= rsShared.getString("document_path") %>">
                                        <button type="submit" class="btn btn-sm btn-outline-success w-100 text-xs d-flex align-items-center justify-content-center gap-1 py-1" style="font-size: 11px;
                                        border-radius: 6px;">
                            ...
                            <td>
                                <div class="d-flex flex-column align-items-center justify-content-center">
                                    ...
                                    <div class="w-100 px-1" style="min-width: 120px;">
                                            ...
                                            border-radius: 3px; background-color: #f1f5f9;">
                                                <%
                                            ...
                                            <button type="button" class="btn btn-link p-0 text-xs text-primary mt-1.5 d-block text-center mx-auto" style="font-size: 11px;
                                            text-decoration: none;"
                                                    onclick="showAIAnalysisDetails('<%= rsShared.getString("org_name").replace("'", "\\'") %>', '<%= aiScore %>', '<%= aiStatus %>', '<%= extRegNo.replace("'", "\\'") %>', '<%= aiSummary.replace("'", "\\'").replace("\n", " ") %>', '<%= passedChecks.replace("'", "\\'") %>', '<%= criticalIssues.replace("'", "\\'") %>')">
							...
							<td><span class="status-badge <%= rsShared.getString("status").toLowerCase() %>"><%= rsShared.getString("status") %></span></td> 
							<td>
							<% if("pending".equalsIgnoreCase(rsShared.getString("status"))) { %> 
							    <div class="d-flex gap-1">
                                    <form action="ApproveNGOServlet" method="POST" style="display:inline;">
                              
                                        <input type="hidden" name="id" value="<%= rsShared.getInt("id") %>">
                                        <input type="hidden" name="email" value="<%= rsShared.getString("email") %>">
                                        
                                        <input type="hidden" name="ngoName" value="<%= rsShared.getString("org_name") %>">
							...
							<% } else if("approved".equalsIgnoreCase(rsShared.getString("status"))) { %> 
                ...
                <table class="admin-table" id="usersTable">
                    ...
                    <tbody>
						...
						<tr>
						    ...
						    <td><%= rsShared.getString("role") %></td> 
						    <td><%= rsShared.getString("city") %></td> 
						    <td><%= rsShared.getString("state") %></td> 
						    <td><span class="status-badge approved">Active</span></td> 
						    <td><button class="btn-action view" onclick="viewUser('<%= rsShared.getString("full_name") %>')"><i class="bi bi-eye"></i></button></td> 
						</tr>
						<%
							}
						} catch(Exception e) { e.printStackTrace();
                    ...
                    <tbody>
                       <%
						...
						<tr>
						    <td><%= srNo++ %></td> 
						    <td><strong><%= rsShared.getString("full_name") %></strong></td> 
						    <td><%= rsShared.getString("skills") != null ? rsShared.getString("skills") : "General" %></td> 
						    <td><%= rsShared.getString("availability") != null ? rsShared.getString("availability") : "Flexible" %></td> 
						    <td><strong><%= rsShared.getInt("total_hours") %></strong> hrs</td> 
						    <td><span class="points-badge"><%= rsShared.getInt("points") %> pts</span></td> 
						    <td><span class="status-badge approved">Active</span></td> 
						</tr>
						<%
							}
            ...
            <div class="table-responsive"> 
                <table class="admin-table" id="donorTable">
                    ...
                    <tbody>
						...
						try {
							int srNo = 1;
                            String sql = "SELECT u.full_name, u.email, u.city, dd.preferred_cause, IFNULL(dd.total_donated, 0.0) AS total_donated " + 
						...
						<tr>
							...
							<td><strong><%= rsShared.getString("full_name") %></strong></td> 
							<td><%= rsShared.getString("email") %></td> 
							<td><%= rsShared.getString("preferred_cause") != null ? rsShared.getString("preferred_cause") : "All Causes" %></td> 
							<td><%= rsShared.getString("city") %></td> 
							<td><strong class="text-success">₹<%= (long)rsShared.getDouble("total_donated") %></strong></td> 
							<td><span class="status-badge approved">Active</span></td> 
						</tr>
						<%
						    }
	        ...
	        <div class="admin-card h-100"> 
               ...
               <div class="report-bars">
	                ...
	                <div class="report-bar-row">
	                    ...
	                    <span class="report-count"><%= pendingNGO 
%></span> 
	                </div>
	                <div class="report-bar-row">
            ...
            <div class="col-md-8">
                <div class="admin-card p-5">
                    ...
                    <div class="text-center mb-4">
                        <div class="profile-avatar-lg mx-auto mb-2" style="width:70px;
                        height:70px; background:#e2f0e7; color:#10B981; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:32px; font-weight:800; align-items:center;">
                            <%= adminName.substring(0,1).toUpperCase() %>
                        </div>
                        <h3><%= adminName %></h3> 
                    ...
                    <form action="UpdateProfileServlet" method="POST" class="space-y-4 row g-3">
                        ...
                        <div class="col-md-12 mt-2">
                            ...
                            <input type="text" name="city" value="Indore" placeholder="e.g.
                            Indore" class="form-control px-3 py-2 text-sm" style="border-radius:10px;">
                        </div>
                        <div class="col-md-12 mt-2">
                            <div class="p-3 bg-light rounded text-muted text-xs" style="font-size:11.5px;
                        ...
                        <div class="col-12 mt-4 text-center">
                            <button type="submit" class="btn btn-success font-bold px-4 py-2 text-sm border-0" style="border-radius:10px;
                            background:#10B981;">
...
<div class="modal fade" id="aiInspectorModal" tabindex="-1" aria-labelledby="aiInspectorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    ...
    border: none; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
        ...
        <div 
         class="row g-3">
          <div class="col-12 bg-white p-3 rounded shadow-sm border border-light">
            <span class="text-xs text-muted fw-bold uppercase" style="font-size: 11px;
            letter-spacing: 0.5px;">EVALUATED ENTITY</span>
          ...
          <div class="col-md-6">
            ...
            <div class="bg-white p-3 rounded shadow-sm border border-light h-100">
              ...
              style="font-size: 11px;"><i class="bi bi-info-circle"></i> Executed instantly via heuristic file extraction</small>
            </div>
          </div>
          ...
          <div class="col-12">
            <div class="bg-white p-3 rounded shadow-sm border border-light">
              ...
              fw-bold" style="font-size: 11px;">EXTRACTED CERTIFICATE REGISTRATION CODE</span>
          ...
          <div class="col-12">
            <div class="bg-white p-3 rounded shadow-sm border border-light">
              ...
              <p class="text-sm text-dark bg-light p-3 border rounded mt-2 mb-0" id="aiModalSummary" style="font-size: 13px;
              line-height: 1.5;">
      ...
      <div class="modal-footer bg-light border-0 py-3">
        ...
        py-2 px-4" style="border-radius: 8px;" data-bs-dismiss="modal">Acknowledge Audit Logs</button>
      </div>
    </div>
  </div>
</div>
...
<div id="floatingAiCopilot" class="floating-copilot">
    ...
    <div class="copilot-window shadow-lg d-none" id="copilotWindow">
        <div class="copilot-header text-white p-3 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <small class="text-success" style="font-size: 10px;">🛡️ Audit System Agent 
                    Active</small>
                </div>
            </div>
        ...
        <div class="copilot-body p-3" id="copilotChatLogs">
            <div class="copilot-msg bot">
                <div class="msg-bubble shadow-sm text-xs">
                    Namaste!
                    Main hoon aapka <strong>CareLink AI Copilot</strong>. Main real-time NGO metadata verification, trust token mappings, aur system databases ko check karne me help kar sakta hoon.
                    <br><br>
        ...
        <div class="copilot-footer border-top p-2 d-flex gap-1 align-items-center">
            <input type="text" class="form-control form-control-sm text-xs" style="border-radius: 6px; font-size: 12.5px;"
            id="copilotInputField" placeholder="Type compliance check..." onkeypress="handleCopilotKeypress(event)">
...
<script>
    ...
    function generateCopilotResponse(query) {
        ...
        setTimeout(() => {
            ...
            } else if (qLower.includes('heuristic') || qLower.includes('scoring') 
            || qLower.includes('logic') || qLower.includes('system')) {
            ...
            } else if (qLower.includes('stat') || qLower.includes('statistic') || qLower.includes('data') ||
